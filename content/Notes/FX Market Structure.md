...
#### Operational risk
The risk of loss due to failed processes, human error, system failure, or fraud.
- Human error: A fat-finger trade can cause big loss or market impact (e.g. a typo extra zero leading to a \$100 million trade instead of \$10 million). Firms implement trading limits per ticket and “sanity checks” in software (a pop-up if an order is abnormally large or far from market price). There have been cases (like the 2016 sterling flash crash was possibly triggered by an input error or algorithm mishap). Traders also use “undo/cancel” features or reach out to counterparties if an obvious mistake trade happens (often the counterparty will honor a request to cancel if it was clearly erroneous and caught immediately, especially on interbank platforms under certain guidelines).
- Systems & IT: If an FX trading system goes down (either the bank’s or a platform’s), that can be costly if the market moves and positions can’t be managed. Firms have redundant systems and disaster recovery sites. Electronic trading firms have multiple connections (if one ECN goes down, use another). Settlement systems outages can also cause fails – CLS has multiple contingency measures.
- Settlement errors: e.g. wrong beneficiary info, or a payment delayed. Straight-through processing (STP) and tools like MT300 confirmations and central matching (e.g. DTCC CTM for FX) ensure trade details match. CLS greatly reduces operational risk by automating multi-currency settlement.
- Fraud and Collusion: There’s risk of employees misusing information (as happened in the fixing scandal where traders colluded). Controls include chat monitoring, enforced time separation around fix windows (some banks require a different team handles fix orders separate from normal trading to avoid conflict), and periodic rotation of staff. For client-facing, some unscrupulous retail brokers historically cheated clients (like using software plugins to cause client trades to fill worse). Regulators now crack down – e.g. the FCA banned some practices and fined firms for asymmetric slippage.
- Rogue Trading: E.g. a trader hides huge losses by falsifying records (Nick Leeson type scenario). To mitigate: daily P&L reconciliation, independent risk oversight, and trade confirmations by back-office with counterparties (so a fake forward won’t find a matching confirmation from a counterparty). In FX, rogue trading is less common because FX desks are very liquid and positions are visible via systems, but it’s not impossible. Having multiple people involved in trade confirmation and settlement adds checks.
